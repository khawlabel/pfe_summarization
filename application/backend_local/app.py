...
async def start():
    ...
    chain = prompt_5w1h | llm | StrOutputParser()
    contxtualisation_chain = prompt_contxtualisation | llm | StrOutputParser()
    answer_chain = prompt_answer | llm | StrOutputParser()
    chain_resumer = ({"context": itemgetter("context"), "language": itemgetter("language")} | prompt_resumer | llm | StrOutputParser())
    chain_traduction_titre  = ({"titre_francais": itemgetter("titre_francais")} | prompt_traduction_titre | llm | StrOutputParser())
    chain_traduction_resume  = ({"resume_francais": itemgetter("resume_francais")} | prompt_traduction_resume | llm | StrOutputParser())
    chain_chat = ({"context": itemgetter("context"),"chat_history": itemgetter("chat_history"), "question": itemgetter("question")} | prompt_chat | llm | StrOutputParser())
