...
def send_verification_email(email: str, token: str):
    ...
    verification_link = f"http://10.0.0.236:3000/verify-email/{token}"
...
def search_support(contenu: str, resume_draft: str, alpha: float = 0.7, beta: float = 0.3, top_k: int = 2):
    ...
    results = []
    points = app.state.support_summaries  # dict retourné par collection.get()

    for i in range(len(points["ids"])):
        source = points["metadatas"][i].get("source")
        resume = points["metadatas"][i].get("resume")
        vec_source = embed_text(source)
        vec_resume = embed_text(resume)
        ...
        resume_txt = points["metadatas"][i].get("resume", points["documents"][i])
        results.append((resume_txt, score))
...
async def start():
    client, embedding_model, vectorstore = init_chroma()
    
            # Récupérer tous les points (résumés) de la collection
    # Charger tous les points (résumés) de la collection
    collection = client.get_collection(CHROMADB_COLLECTION_SUPPORT_4)
    points = collection.get(include=["documents", "metadatas", "embeddings"])  
...
async def upload_and_store_file(
    ...
    for f in file:
        ...
        try:
            ...
            collection = app.state.client.get_or_create_collection(CHROMADB_COLLECTION)

            collection.add(
                documents=contextualized_versions,
                metadatas=metadatas,
                ids=[f"{user_id}_{file_name}_{idx}" for idx in range(len(contextualized_versions))]
            )
...
async def chat_stream(data: ChatRequest):
    ...
    bm25_retriever = getattr(app.state, "bm25_retriever", None)

    # ✅ Recherche dans Chroma
    collection = app.state.client.get_collection(CHROMADB_COLLECTION)
    query_embedding = embed_text(user_input)

    results = collection.query(
        query_embeddings=[query_embedding],
        n_results=app.state.nbr_fichiers * 2,
        include=["documents", "metadatas"]
    )

    # Reconstruction des documents sous forme de "doc-like"
    vectordocs = [
        Document(
    ...
    context_chunks = []
    for doc in vectordocs + bm25docs:
        key = (doc.metadata.get("file_name"), doc.metadata.get("chunk_id"))
        if key not in seen:
            seen.add(key)
...
async def reset_app_state():
    ...
    # Supprimer tous les documents (en récupérant leurs ids)
