...
french_ocr_prompt = """Tu es un assistant spécialisé dans la correction et l'amélioration des textes extraits via OCR en français.
                  ...
                  - Corrige précisément les erreurs de lettres sans modifier complètement un mot. Par exemple, si le texte
                    contient "ft th", corrige-le en "ftth" sans changer la structure du mot.
                  - Supprime les éléments non pertinents, tels que :
                        - Les dates de publication (ex : « Publié le : », Le : samedi 28 sept. 2024 », « 18/2/2025 »,
                          « dimanche 13 oct. 2024 », « 13 octobre 2024 », « 18/2/2025 », « 18:40 »).
                        - Les liens et URL (ex : « https:// », « www. », « .dz »).
                        - Les noms de journaux et identifiants techniques (ex : « LA NOUVELLE RÉPUBLIQUE », « OUEST TRIBUNE », 
                        « ALGERIE CONFLUENCES », etc.).
...
def clean_text_extracted_from_pdf(file_path, lang):
    ...
    if lang == "fr":
        system_prompt = french_ocr_prompt
    elif lang == "ar":
        system_prompt = arabic_ocr_prompt
    else:
        raise ValueError("Langue non supportée. Utilisez 'fr' pour le français ou 'ar' pour l'arabe.")
    ...
    # Lire la réponse progressivement
    response_text = ""
    for chunk in completion:
        if chunk.choices and chunk.choices[0].delta.content:
            response_text += chunk.choices[0].delta.content

    # Supprimer les balises inutiles si présentes
